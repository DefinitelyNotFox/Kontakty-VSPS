...
import { Check, X, RotateCcw, Sparkles, Image, User, Layers, StickyNote, Eye } from 'lucide-react';
...
export default function QuizGame({
  ...
  return (
    <div style={{ maxWidth: '640px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div className="glass-panel" style={{ borderRadius: 'var(--radius-lg)', padding: '1rem 1.25rem', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
        <div style={{ display: 'flex', gap: '0.4rem' }}>
          ...
          <button
            ...
            style={{ padding: '0.4rem 0.7rem', fontSize: '0.8rem' }}
          >
            <Layers size={14} /> Smíšený režim
          </button>
        </div>

        {/* Simplified Count Indicator: e.g. "16 ze 16" */}
      ...
      {/* Main Question Card */}
      <div className="glass-panel" style={{ borderRadius: 'var(--radius-lg)', padding: '2rem 1.5rem', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', boxShadow: 'var(--shadow-lg)' }}>
        ...
        {currentMode === 'photoToName' && (
          <>
            ...
            <div className="avatar-wrapper" style={{ width: '150px', height: '150px', border: '3px solid var(--accent-primary)', boxShadow: 'var(--shadow-glow)' }}>
              {targetPhoto ? (
                <img src={targetPhoto} alt="Kolega" className="avatar-img" />
              ) : (
                <div className="avatar-badge" style={{ background: getAvatarGradient(currentTarget.id), fontSize: '3rem' }}>
            ...
            {targetNote && (
              <div className="user-note-box" style={{ maxWidth: '400px', margin: '0 auto' }}>
                <StickyNote size={15} style={{ flexShrink: 0 }} /> <span>Nápověda: {targetNote}</span>
              </div>
            )}

            {/* 4 Name Options Grid */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.85rem', width: '100%', marginTop: '0.5rem' }}>
              {options.map((option) => {
                let btnStyle = {
                ...
                return (
                  <div key={option.id} style={{ display: 'flex', width: '100%', gap: '0.35rem' }}>
                    <button
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', flexShrink: 0 }}>
                        {isAnswered && option.id === currentTarget.id && <Check size={18} />}
                        {isAnswered && selectedAnswer && option.id === selectedAnswer.id && option.id !== currentTarget.id && <X size={18} />}
                      </div>
                    </button>
        ...
        {currentMode === 'nameToPhoto' && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem', width: '100%', maxWidth: '440px', margin: '0.5rem auto 0 auto' }}>
              {options.map((option) => {
                ...
                return (
                  <div key={option.id} style={{ position: 'relative' }}>
                    ...
                    >
                      ...
                      {isAnswered && (
                        <div style={{ position: 'absolute', inset: 0, background: isCorrect ? 'rgba(16, 185, 129, 0.4)' : isSelected ? 'rgba(239, 68, 68, 0.4)' : 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
                          {isCorrect && <Check size={36} strokeWidth={3} />}
                          {isSelected && !isCorrect && <X size={36} strokeWidth={3} />}
                        </div>
                      )}
                    </div>
                    ...
                    {isAnswered && (
                      ...
                      >
                        <Eye size={14} /> <span>{option.name}</span>
                      </button>
                    )}
                  </div>
                );
        ...
        {isAnswered && (
          <button
            ...
            onClick={handleNext}
            style={{ marginTop: '0.5rem', padding: '0.85rem 2rem', fontSize: '1rem', width: '100%', animation: 'fadeIn 0.2s ease' }}
          >
            Další otázka ➔
          </button>
        )}
      ...
      {previewingContact && (
        <div className="modal-overlay" onClick={() => setPreviewingContact(null)}>
          <div className="modal-content" style={{ maxWidth: '340px', textAlign: 'center', padding: '1.5rem' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <h3 style={{ margin: 0, fontSize: '1.1rem' }}>{previewingContact.name}</h3>
              <button className="icon-button" onClick={() => setPreviewingContact(null)}>
                <X size={18} />
              </button>
            </div>
            
            <div className="avatar-wrapper" style={{ width: '180px', height: '180px', margin: '0 auto 1rem auto', border: '3px solid var(--accent-primary)', boxShadow: 'var(--shadow-glow)' }}>
              {customPhotosMap[previewingContact.id] || previewingContact.photoUrl ? (
                ...
                <div className="avatar-badge" style={{ background: getAvatarGradient(previewingContact.id), fontSize: '3.5rem' }}>
                  {getInitials(previewingContact.name)}
                </div>
              )}
            </div>

            <p style={{ color: 'var(--accent-primary)', fontSize: '0.9rem', fontWeight: 600, margin: '0 0 0.5rem 0' }}>
